export default {
  verified: {
    ...
    files: {
      ...
      html: "HTML copy",
      json: "JSON record",
      note: "Written by the engine when the run finished, from the same record this page renders. The PDF is the one to attach to a review; the HTML copy opens anywhere, without STRhub.",
    },
    footnote:
    ...
    dispute: {
      ...
      mailSubject: "About the STRhub Verified attestation",
    },
    group: {
      commits: "{n} verifications",
      commitSingular: "1 verification",
    },
    history: {
      ...
      older: {
        title: "You are reading an older commit",
        body: "This result is for {what}. The newest verified commit of this tool is {newest}.",
        commitMade: "commit {sha}, made {committed}",
        commit: "commit {sha}",
        cta: "See the newest",
      },
    },
    ...
    certificate: {
      section: {
        ...
        why: "Why it stopped, and what it reported",
        reported: "What the tool reported on the way",
        command: "Exact run command",
        metadata: "Reproducibility metadata",
        outOfScope: "Out of scope",
        limitations: "Limitations",
        scope: "Scope and disclaimers",
      ...
      cover: {
        ...
        reached: "Reached",
        permalink: "Permalink",
        notPublished: "Not published. This is a test run.",
      ...
      outOfScopeLead: "This report does not evaluate any of the following:",
    },
    headline: {
    ...
    asIs: {
      heading: "As it is in the repository",
      fullRun: "This run, in full",
      details: "Run details",
    },
    strhubDid: {
      ...
      reached: "With those changes, the run reached:",
      ...
      noLabelChange: "This does not change the tool's label. The label is what happens as it is in the repository.",
      readRun: "Read that run in full",
      rowNote: "With STRhub's own recipe, at this commit: {label}",
      notice: {
        title: "Not verified as documented",
        body: "This is the full report of a run of a recipe STRhub wrote, not of the repository's own instructions. It shows how far the tool can be made to run; it does not say whether a first-time user following the README gets there, and it is not the tool's label.",
        seeDocumented: "See the result as it is in the repository: {label}",
        noDocumented: "STRhub has not yet run this tool from its own instructions. The next documented refresh will, and that result will be the label.",
      },
    },
    ...
    content: {
      ...
      format: "Format",
      maxDepth: "Deepest locus",
      panelCoverage: "Panel loci called",
      panelCoverageValue: "{hit} of {given}",
      notCalled: "Not called:",
      notCalledHint: "Loci the regions file asked for that the output does not name. The auto-diagnostics below usually say why.",
      ioChecksLabel: "Checks on the file",
      ioCheck: {
        exists: "exists",
        non_empty: "not empty",
        confined: "written inside the output directory",
        parses: "parses as {format}",
        min_records: "has records",
        must_contain: "contains the declared text",
      },
      depthHeading: "Read depth per locus",
      depthNote: "Reads the tool reported at each locus it called, deepest first. Depth reflects the coverage of the reference slice as much as the tool; it is here so a reader can see the output is real, not to rank loci.",
      showAll: "Show all {n}",
      showFewer: "Show fewer",
    },
    run: {
      heading: "Command that ran",
      note: "Executed verbatim inside the container, at the pinned commit. Paths under /data are STRhub's mounts: the input sample, the reference genome and the output directory.",
    },
    data: {
    ...
    trial: {
      ...
      evidenceClaim: {
        ...
        documented_input: "Documented input",
        author_recommendation: "Author's recommendation",
      ...
      recipeTitle: "Environment it ran in",
